...
class Database:
    ...
    async def _load_cache(self):
        ...
        for row in rows:
            ...
            self.users[uid]["balance"] = dict(
                _EMPTY_BALANCE
            )

        async with self._conn.execute(
            "SELECT user_id, currency, amount FROM balances"
        ) as cur:
        ...
        for row in rows:
            ...
            if uid in self.users:
                ...
                self.users[uid].setdefault(
                    "referrals",
                    []
                )

                self.users[uid]["referrals"].append(
                    row["referee_id"]
                )

        async with self._conn.execute(
            "SELECT deal_id, data FROM deals"
        ) as cur:
    ...
    async def add_balance(
        ...
        async with _LOCK:
            ...
            await self._add_transaction_locked(
                uid,
                "credit",
                currency,
                amount,
    ...
    async def _add_transaction_locked(
        self,
        uid: str,
        tx_type: str,
        currency: str,
